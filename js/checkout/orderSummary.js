...
function deliveryOptionHTML(productId, cartItem) {
  ...
  deliveryOptions.forEach((option, index) => {
    deliveryOptionOutput += `<div class="delivery-option js-deliver-option">
                  <input
                    type="radio"
                  ...
                  />
                  <div>
                    <div class="delivery-option-date">${today
                      .add(option.deliveryDate, "days")
                      .format("dddd, MMM D")}</div>
                    <div class="delivery-option-price">${
                      option.deliveryPrice === 0
                        ? "FREE Shipping"
                        : `$${formatCurrency(option.deliveryPrice)} Shipping`
                    }</div>
                  </div>
                </div>`;
  });
  return deliveryOptionOutput;
}
